...
const log1 = require("../controllers/log").log1;
var token = {
    createToken: function(obj, timeout) {
        var obj2 = {
            data: obj, //payload
            created: parseInt(Date.now() / 1000), //token生成的时间的，单位秒
            exp: parseInt(timeout) || 5 //token有效期
        };
        //payload信息
    ...
    decodeToken: function(token) {
        ...
        try {
            payload = JSON.parse(
                Buffer.from(decArr[0], "base64").toString("utf8")
            );
        } catch (e) {
            return false;
        }
...
// MD5 Hash
var secret_md5 = function(mydata) {
    var aftermd5 = crypto
        .createHash("md5")
        .update(mydata)
        .digest("hex");
...
exports.token = token;
exports.secret = function(mydata) {
    var aftermd5 = crypto
        .createHash("md5")
        .update(mydata)
        .digest("hex");
