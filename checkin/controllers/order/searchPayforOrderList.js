...
WHERE `
    if (serchinput && serchinput != "") {
      ...
      OR p.customerId = (SELECT lr_renyuan FROM customer WHERE c_name = "${serchinput}")) `
      mysql += sql1
    } else {
      var sql1 = ""
      mysql += sql1
...
LEFT JOIN user u ON u.u_id = p.fk_id WHERE `
  ...
  OR o.customerId = (SELECT lr_renyuan FROM customer WHERE c_name = "${serchinput}")) `
      mysql += sql1
    } else {
      var sql1 = ""
      mysql += sql1
    ...
    if (startDate && startDate != "" && endDate && endDate != "") {
      var sql4 = ` o.pay_date >= "${startDate}" AND o.pay_date < "${endDate}" `
      if (sql3 == "") {
        mysql += sql4
      } else {
        mysql += "AND" + sql4
      ...
      var sql4 = ` o.pay_date LIKE "%${startDate}%" `
      if (sql3 == "") {
        mysql += sql4
      } else {
        mysql += "AND" + sql4
