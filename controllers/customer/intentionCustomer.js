    ...
    */
    if( Level === 3 ){
        let sellecttotal = new Promise((resolve,reject)=>{
            ...
            WHERE c.come_rate = 1  AND c.c_mean = "意向客户" AND ((c.lr_renyuan in (SELECT u_id FROM user WHERE superior = ${userID}) OR c.lr_renyuan = ${userID}) OR (c.kf_tel IN (SELECT u_username FROM user WHERE superior = ${userID})))`
        ...
        sellecttotal.then((result)=>{
            ...
            let sql = `SELECT
                ...
                WHERE c.come_rate = 1  AND c.c_mean = "意向客户" AND (c.lr_renyuan in (SELECT u_id FROM user WHERE superior = ${userID}) OR c.lr_renyuan = ${userID})
        ...
        let sellecttotal = new Promise((resolve,reject)=>{
            let total = `SELECT c.customerId FROM customer c
            LEFT JOIN orderyuegui oo ON oo.customerId = c.customerId
            WHERE c.come_rate = 1  AND c.c_mean = "意向客户" AND (c.lr_renyuan  = ${userID} OR c.kf_tel = "${username}")`
            query(total,(err,results,fields)=>{
                if(err) return reject(err)
                resolve(results.length)
            })
