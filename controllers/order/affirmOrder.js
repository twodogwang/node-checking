...
exports.affirmOrder = (req, res) => {
  ...
  /* 
    确认订单，确认顺序： 1，风控专员，营销主管，
      2，财务专员，财务有两个确认，一个是反费，一个是服务费
      3，监督专员
      前面的所有确认都确认完之后，监督才能确认
      如果是财务确认服务费，会传过来 affirmType 并且 affirmType ==1
      否则就是反费确认
  */
  ...
  if (Level == 2) {
    ...
    if (affirmType == 1) {
      var sql2 = `caiwuSure2 = '1',caiwuBZ = CONCAT((CASE WHEN caiwuBZ = '' OR caiwuBZ IS NULL THEN ' ' ELSE  caiwuBZ END) ,"\n${time}确认服务费:${affirmBz}")`
      var canChecksql = `SELECT sellSure FROM payfororder`
      var beforCheck = `SELECT caiwuSure2 FROM payfororder`
    } else {
      var sql2 = `caiwuSure = '1',caiwuBZ = CONCAT((CASE WHEN caiwuBZ = '' OR caiwuBZ IS NULL THEN ' ' ELSE  caiwuBZ END) ,"\n${time}确认返费:${affirmBz}")`
