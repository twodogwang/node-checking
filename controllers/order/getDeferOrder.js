...
exports.getDeferOrder = function (req, res) {
  console.log("getDeferOrder")
  ...
  */
  switch (Level) {
    case 6:
      var pageSql = `SELECT id FROM orderYuegui WHERE ifaccept = '1' AND protype = "暂缓" AND MangerId = ${UserId} `
      var sqltatil = ` AND MangerId = ${UserId}`
      break;
    case 1:
    case 2:
    case 5:
      var pageSql = `SELECT id FROM orderYuegui WHERE ifaccept = '1' AND protype = "暂缓" `
      var sqltatil = ``
      break;
    case 7:
      var pageSql = `SELECT id FROM orderYuegui WHERE ifaccept = '1' AND ifpass = '1' AND protype = "暂缓" AND fk_id = ${UserId} `
  ...
  pagePromise.then((result) => {
    ...
    if (result === 0) {
      ...
      let sql = `SELECT
            ...
            WHERE
            ifaccept = '1' AND ifpass = '1'  AND ifback = '0' `;
      sql += sqltatil
      sql += sqltatil1
      console.log(sql)
