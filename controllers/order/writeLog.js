...
const fs = require('fs');
/* 
  填写日志，分为 if_sign 签单和不签单两种情况 
  签单了要发照片，
  写这个的时候遇到了一个问题，就是不带图片的时候是走formidable插件，不带图片的时候并不走formidable插件
  然后现在的逻辑上 if_sign == '0' 就按照不走 formidable插件 来写，
  else 就按走formidable插件 来写，那么问题就是！！！如果if_sign == '1' 但是他没带图片！！！会怎么样
*/
