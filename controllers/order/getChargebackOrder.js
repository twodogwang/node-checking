...
exports.getChargebackOrder = function (req, res) {
  console.log("getChargebackOrder")
  ...
  */
  switch (Level) {
    case 6:
      var pageSql = `SELECT id FROM orderYuegui WHERE ifaccept = '1' AND protype = "退单" AND MangerId = ${UserId} `
      var sqltatil = ` AND MangerId = ${UserId}`
      break;
    case 1:
    case 2:
    case 5:
      var pageSql = `SELECT id FROM orderYuegui WHERE ifaccept = '1' AND protype = "退单" `
      var sqltatil = ``
      break;
    case 7:
      var pageSql = `SELECT id FROM orderYuegui WHERE ifaccept = '1' AND ifpass = '1' AND protype = "退单" AND fk_id = ${UserId} `
